...
public class HuaWeiComputerTestSimple {
	...
	public static void getPasswordSecurityLevel() {
		Scanner sc = new Scanner(System.in);
		while (sc.hasNext()) {
			String[] inputPwd = sc.nextLine().split("");
			int score = 0;
	...
	public static void argumentParsing() {
		Scanner sc = new Scanner(System.in);
		while (sc.hasNextLine()) {
			String[] string = sc.nextLine().split(" ");// 根基空格拆分数组
			String tempString = "";
			boolean flag = false;
	...
	public static void inversionInt() {
		Scanner sc = new Scanner(System.in);
		while (sc.hasNextInt()) {
			String num = sc.nextInt() + "";// 将整型转化成字符型
